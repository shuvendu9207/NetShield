...
def run_rf_pipeline(processed_dir: str = PROCESSED_DATA_DIR):
    """Runs the training pipeline for Random Forest."""
    # 1. Load Dataset
    ...
    # Note: Model persistence (saving model, scaler, encoder) will be fully implemented in Module 6.
    # We will save intermediate assets for testing purposes.
    os.makedirs(MODELS_DIR, exist_ok=True)
    # We will save these temporary artifacts so the model can be tested/run
    joblib.dump(rf_model, os.path.join(MODELS_DIR, "rf_model.pkl"))
    joblib.dump(scaler, os.path.join(MODELS_DIR, "scaler.pkl"))
    joblib.dump(le, os.path.join(MODELS_DIR, "label_encoder.pkl"))
    logger.info("Saved temporary Random Forest artifacts to models/ directory.")
